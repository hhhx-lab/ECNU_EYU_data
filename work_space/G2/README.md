...
5. 已补充 `diffusion_quality_metrics_template.csv` 和 `synthetic_generation_manifest_template_g1.csv`，用于后续数据报告和 G1 smoke run 接收。
6. 已完成本机可执行的训练前数据准备，结果保存到 [results/](results/)；GPU 训练、在线 batch 生成、nnU-Net 预处理和大规模 synthetic NIfTI 生成暂缓到训练机执行。
