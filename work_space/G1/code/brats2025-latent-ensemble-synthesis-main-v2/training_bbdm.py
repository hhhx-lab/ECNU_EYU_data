...
def train(
    ...
    train_dataloader = instantiate_dataset(
        ...
        lesion_weights_path=_lw_path if getattr(args_train, "use_lesion_weights", False) else None,
        )
    ...
    def loss_pt_per_channel(
        x,
        y,
        to_mod_index,
        channel_importance_weights=None,
        target_modality_weight=1,
        extra_modalites_weight=1,
        voxel_weights=None,
    ):
        ...
        for i in range(n_modalities):
            ...
            for j in range(n_latent_channels):
                ...
                if voxel_weights is None:
                    ...
                    weights = voxel_weights.to(dtype=x_ch.dtype)
                    weighted_error = (x_ch - y_ch).square() * weights
                    ch_loss = weighted_error.sum() / weights.sum().clamp_min(1.0)
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            with _autocast(enabled=args_train.amp):
                ...
                healthy_weights = (1 - attmask_64_64_40) * healthy_cond[:, None, None, None, None]
                tumor_weights = tumor_weight_mask * tumor_cond[:, None, None, None, None]
                loss_healthy = loss_pt_per_channel(noise_pred.float(),
                                                   objective.float(),
                                                   __to_mod_index,
                                                   channel_importance_weights=args_train.channel_importance_weights,
                                                   target_modality_weight=args_train.target_modality_weight,
                                                   extra_modalites_weight=args_train.extra_modalites_weight,
                                                   voxel_weights=healthy_weights)
                loss_tumor = loss_pt_per_channel(noise_pred.float(),
                                                    objective.float(),
                                                    __to_mod_index,
                                                    channel_importance_weights=args_train.channel_importance_weights,
                                                    target_modality_weight=args_train.target_modality_weight,
                                                    extra_modalites_weight=args_train.extra_modalites_weight,
                                                    voxel_weights=tumor_weights)
...
args_train = {
    ...
    # ---- per-lesion V weighting (set use_lesion_weights=True after running precompute_lesion_weights.py)
    "use_lesion_weights": False,
    ...
    "channel_importance_weights": load_channel_importance_weights([0.50995545, 0.12407539, 0.20278341, 0.16318575]), # obtained from compute_weights.py when available
    # "channel_importance_weights": None,
