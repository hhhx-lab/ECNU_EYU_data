...
# Slurm may set CUDA_VISIBLE_DEVICES to a physical GPU id; PyTorch remaps the
# visible device to cuda:0 inside the job.
...
def instantiate_unconditioned_models(device, reserved_space=0):
    networks_config = {
        ...
        "diffusion_unet_def": copy.deepcopy(configs.NETWORKS_CONFIG["encdec"]["unet"]),
    }
...
def train(
    ...
    if len(train_dataloader) == 0:
        raise RuntimeError("The EncDec train split produced an empty dataloader.")
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            # --- seg-guided loss (V-weighted tumor loss) ---
            # Extract attention mask (tumor regions in latent space)
            attmask_key = f"attmasks_{'_'.join(str(x) for x in args_train.attmasks_shapes_list[0])}"
            attmask = batch.get(attmask_key, None)
            if attmask is not None:
                attmask = attmask.to(device)

            # Extract per-lesion V-weight mask (if available)
            lesion_w_key = f"lesion_weights_{'_'.join(str(x) for x in args_train.attmasks_shapes_list[0])}"
            lesion_weights = batch.get(lesion_w_key, None)
            if lesion_weights is not None:
                lesion_weights = lesion_weights.to(device)

            use_seg_loss = (attmask is not None and
                            getattr(args_train, "use_seg_loss", False))
            ...
            with _autocast(enabled=args_train.amp):
                ...
                if use_seg_loss:
                    # ---- Seg-guided loss (healthy + tumor split) ----
                    pred_f = missing_modality_pred.float()
                    target_f = to_modality.float()

                    # Healthy loss (background / non-tumor brain)
                    healthy_mask = 1.0 - attmask
                    diff_healthy = (pred_f - target_f) ** 2 * healthy_mask
                    n_healthy = max(int(healthy_mask.sum()), 1)
                    loss_healthy = diff_healthy.sum() / n_healthy

                    # Tumor loss (with optional V weighting)
                    tumor_weight = lesion_weights if lesion_weights is not None else attmask
                    diff_tumor = (pred_f - target_f) ** 2 * tumor_weight
                    n_tumor = max(int(attmask.sum()), 1)
                    loss_tumor = diff_tumor.sum() / n_tumor

                    # Adaptive lambda_tumor from healthy/tumor ratio
                    lambda_tumor = n_healthy / n_tumor
                    lambda_tumor = max(3.0, min(30.0, lambda_tumor))

                    loss = (loss_healthy + lambda_tumor * loss_tumor) / args_train.gradient_accumulation_steps
                else:
                    # ---- Original uniform MSE loss (commented, not deleted) ----
                    # loss = loss_pt(missing_modality_pred.float(), to_modality.float()) / args_train.gradient_accumulation_steps
                    loss = loss_pt(missing_modality_pred.float(), to_modality.float()) / args_train.gradient_accumulation_steps
            ...
            if gradient_accumulation_count % args_train.gradient_accumulation_steps == 0:
                ...
                if use_seg_loss:
                    writer.add_scalar("Loss/healthy", loss_healthy.item(), global_step)
                    writer.add_scalar("Loss/tumor", loss_tumor.item(), global_step)
                    writer.add_scalar("Loss/lambda_tumor", lambda_tumor, global_step)
...
args_train = {
    ...
    "output_path": os.path.abspath(os.environ.get(
        "G1_ENCDEC_OUTPUT_PATH", os.path.join(configs.PATH_TRAINING, "endec")
    )),
...
"latents_path": os.path.join(configs.PATH_DATA, "latents"),
    ...
    # ---- seg-guided loss (set use_seg_loss=True after running precompute_lesion_weights.py)
    "use_seg_loss": False,

    # training configuration (540 epochs × 1489 subjects / batch_size=6 ≈ 134000)
    "max_train_steps": int(os.environ.get("G1_ENCDEC_MAX_STEPS", "134000")),
    ...
    # ---- Training stability
    "batch_size": int(os.environ.get("G1_ENCDEC_BATCH_SIZE", "6")),
    ...
    # validation
    "val_interval": int(os.environ.get("G1_ENCDEC_VAL_INTERVAL", "1000")),
    "initial_val": True,
