...
class PathsLoader:
    def __init__(self, df_path, data_path, load_latents=True, load_seg=False, attmasks_path=None, attmasks_shapes_list=None, load_org_img=False, lesion_weights_path=None):
        ...
        self.lesion_weights_path = lesion_weights_path
    ...
    def get_data_by_split(self, split="train"):
        ...
        for i, row in complete_df.iterrows():
            ...
            if all(not pd.isna(row[m]) and str(row[m]).strip() for m in required_modalities):
                s_id = row["id"]

                if self.load_org_img:
                    t1n_latent_name = row["t1n"]
                    t1c_latent_name = row["t1c"]
                    t2_latent_name = row["t2w"]
                    flair_latent_name = row["t2f"]
                else:

                    # t1n,t1c,t2,flair,seg
                    t1n_path = os.path.basename(row["t1n"]).split(".")[0]
                    t1c_path = os.path.basename(row["t1c"]).split(".")[0]
                    t2w_path = os.path.basename(row["t2w"]).split(".")[0]
                    t2f_path = os.path.basename(row["t2f"]).split(".")[0]
                    # seg_path = row["seg"].split(".")[0]

                    t1n_latent_name = self.create_latent_name(t1n_path)
                    t1c_latent_name = self.create_latent_name(t1c_path)
                    t2_latent_name = self.create_latent_name(t2w_path)
                    flair_latent_name = self.create_latent_name(t2f_path)

                _instance = {}
                _instance["t1n"] = os.path.join(self.data_path, s_id, t1n_latent_name)
                _instance["t1c"] = os.path.join(self.data_path, s_id, t1c_latent_name)
                _instance["t2w"] = os.path.join(self.data_path, s_id, t2_latent_name)
                _instance["t2f"] = os.path.join(self.data_path, s_id, flair_latent_name)
                if self.load_seg:
                    _instance["seg"] = os.path.join(
                        configs.PATH_INPUT, s_id, os.path.basename(row["seg"])
                    )
                ...
                if self.lesion_weights_path is not None and self.attmasks_shapes_list is not None:
                    __lw_path_names = []
                    for i, lw_shape in enumerate(self.attmasks_shapes_list):
                        __lw_path_names.append(f"{self.lesion_weights_path}/{s_id}/{s_id}_lesion_weights_{'_'.join(str(x) for x in lw_shape)}.npy")
                    _instance["lesion_weights"] = __lw_path_names
...
class PrepareDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                ...
                attmasks_path=None, # path to the attention masks
                 ...
                 lesion_weights_path=None, # path to pre-computed per-lesion V-weight masks
                 ):
        ...
        if not self.load_only_latents:
            self.load_org_img = True
            paths_loader_imgs = PathsLoader(
                df_path,
                configs.PATH_INPUT,
                load_latents=False,
                load_org_img=True,
    ...
    def __getitem__(self, index):
        ...
        if self.load_attmasks:
            ...
            for i, attmask_path in enumerate(attmasks_paths):
                if os.path.exists(attmask_path):
                    attmask = np.load(attmask_path)
                else:
                    attmask = np.zeros(self.attmasks_shapes_list[i], dtype=np.float32)
        ...
        # Load per-lesion V-weight masks (if available)
        if "lesion_weights" in instance_latents:
            lw_paths = instance_latents["lesion_weights"]
            for i, lw_path in enumerate(lw_paths):
                if os.path.exists(lw_path):
                    lw = np.load(lw_path)
                    lw = np.expand_dims(lw, axis=0)  # add channel dimension
                    sample[f"lesion_weights_{'_'.join(str(x) for x in self.attmasks_shapes_list[i])}"] = torch.from_numpy(lw)
...
def collate_fn(samples):
    ...
    # find all the key containing "attmasks_"
    ...
    # stack all the lesion weight masks with the same shape
    lw_keys = [key for key in samples[0].keys() if key.startswith("lesion_weights_")]
    for key in lw_keys:
        lws = torch.stack([example[key] for example in samples])
        lws = lws.to(memory_format=torch.contiguous_format).float()
        samples_dict[key] = lws
