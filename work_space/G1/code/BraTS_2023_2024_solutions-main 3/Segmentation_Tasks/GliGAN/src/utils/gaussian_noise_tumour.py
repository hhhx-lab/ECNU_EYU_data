...
class GaussianNoiseTumour(MapTransform):
    ...
    def __call__(self, data):
        ...
        # Guard against coordinate/geometry errors before they become silent
        # background-only training samples.
        if scan_crop.numel() == 0 or not torch.any(scan_crop != 0):
            scan_shape = scan_data.shape
            raise RuntimeError(
                f"Empty or all-zero scan crop produced for lesion. "
                ...
                f"x_pad={x_pad:.1f}, y_pad={y_pad:.1f}, z_pad={z_pad:.1f}"
            )
    ...
    def add_gaussian_noise_tumour(self, scan, label, is_zscore=False):
        ...
        if is_zscore:
            # Preserve zero padding, but always replace labelled tumour voxels.
