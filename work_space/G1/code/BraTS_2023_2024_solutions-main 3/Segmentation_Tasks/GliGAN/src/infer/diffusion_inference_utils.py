...
def make_diffusion_coefficients(n_steps=1000, beta_schedule="cosine", device="cpu",
                                noise_schedule="cosine", sigma_data=0.5, sigma_max=50.0,
...
def sample_tumour_diffusion_inpaint(model, noisy_scan, label_cond, n_steps, betas,
    ...
    noisy_scan:   [1, 1, S, S, S]  — scan with Gaussian noise in tumour (S = crop_size)
    label_cond:   [1, C_label, S, S, S]  — multi-channel tumour label
    Returns:      [1, 1, S, S, S]  — reconstructed clean scan
    ...
    tumour_mask = (label_cond.sum(dim=1, keepdim=True) > 0).float()  # [1, 1, S, S, S]
    ...
    if method == 'edm_heun':
        if noise_schedule_cfg is None or noise_schedule_cfg.sigma_data is None:
            raise ValueError("EDM inpainting requires noise_schedule_cfg")
        return _diffusion_utils.sample_edm(
            model=model,
            shape=tuple(noisy_scan.shape[1:]),
            cond=label_cond,
            schedule_cfg=noise_schedule_cfg,
            device=device,
            num_steps=sampling_steps or 18,
            cfg_weight=cfg_weight,
            known_scan=noisy_scan,
            known_mask=known_mask,
        )

    if method == 'lognsr_ode':
        raise ValueError("logsnr_ode inpainting is not implemented; use edm_heun")
    ...
    # ============================ DDIM 分支 — 新增加速 inpainting 采样 ============================
        ...
        raise ValueError(
            f"Unknown method: '{method}'. Use 'ddpm', 'ddim', or 'edm_heun'.")
...
# ---------------------------------------------------------------------------
# 向量化后处理函数
# 原 GAN 版本的后处理使用 Python 三重 for 循环遍历每个体素
...
def correct_background_vec(healthy_crop_pad, imgs_recon):
    ...
    参数:
        healthy_crop_pad: (S,S,S) numpy 数组，健康扫描的 crop_size³ 区域
                          （已归一化到 [-1,1]，脑外区域值为 -1）
        imgs_recon:       (S,S,S) numpy 数组，扩散模型生成的肿瘤扫描
    返回:
        np.ndarray: 修正后的 (S,S,S) 数组
    """
...
def correct_label_vec(label, healthy_scan, original_label):
    ...
    healthy_scan 中脑外值为 -1 或 0 的体素对应的标签被清零，
    与 original_label 重叠的标签也被清零。
    ...
    参数:
        label:           (S,S,S) 要修正的标签
        healthy_scan:    (S,S,S) 健康扫描（归一化后，脑外值为 -1 或 0）
        original_label:  (S,S,S) 原有肿瘤标签
    返回:
        np.ndarray: 修正后的 (S,S,S) 标签
    """
...
def get_inten_coord_vec(healthy_scan_crop, original_label_crop, noise):
    """
    向量化版本：在 crop_size³ 立方体的 6 个面上搜索"干净"参照点（脑内、非肿瘤、无噪声）。
    ...
    参数:
        healthy_scan_crop:  (S,S,S) 健康扫描的 crop_size³ 区域
        original_label_crop:(S,S,S) 原有标签
        noise:              (S,S,S) 噪声掩码（肿瘤区域非零）
    返回:
    ...
    clean_mask = is_brain & is_clean_label & is_clean_noise  # (S,S,S)

    s = healthy_scan_crop.shape[0] - 1  # last index (works for any crop_size)
...
def linear_interpolation_vec(final_recons, healthy_scan_crop,
    ...
    参数:
        final_recons:      (S,S,S) 扩散模型生成的肿瘤扫描
        healthy_scan_crop: (S,S,S) 原始健康扫描
        untouch_x_axis:    参照点 x 坐标
        untouch_y_axis:    参照点 y 坐标
        untouch_z_axis:    参照点 z 坐标
    返回:
        np.ndarray: 强度校正后的 (S,S,S) 数组
    """
...
def add_gaussian_noise_tumour_vec(scan, label):
    ...
    原函数使用三重 for 循环遍历每个体素调用 torch.randn，
    这里用 numpy 向量化替代（np.random.randn 一次性生成所有噪声）。

    参数:
        scan:  (S,S,S) numpy 数组，归一化到 [-1,1] 的健康扫描
        label: (S,S,S) numpy 数组，整型标签（非零=肿瘤区域）
    返回:
        scan_noisy: (S,S,S) 肿瘤区域被高斯噪声替换后的扫描
        noise:      (S,S,S) 噪声掩码（肿瘤区域=1，其他=0）
    """
...
def add_gaussian_noise_tumour_zscore(scan, label):
    """Z-score variant: add N(0,1) noise to tumour region, no [-1,1] rescaling.

    Assumes *scan* is already z-score normalized (~N(0,1) in brain, 0=background).
    Noise std=1.0 matches the data scale.  No post-noise rescaling is applied,
    so the output stays in z-score space for the diffusion model trained with
    ``--normalization zscore``.

    Args:
        scan:  (S,S,S) numpy array, z-score normalized healthy scan
        label: (S,S,S) numpy array, integer label (non-zero = tumour)

    Returns:
        scan_noisy: (S,S,S) tumour region replaced with N(0,1) noise
        noise_mask: (S,S,S) float32, 1 in tumour region
    """
    scan_noisy = np.copy(scan).astype(np.float32)
    tumour_mask = (label != 0)
    noise = np.zeros_like(scan, dtype=np.float32)
    n_tumour = int(tumour_mask.sum())
    if n_tumour > 0:
        noise[tumour_mask] = np.random.randn(n_tumour).astype(np.float32)
    # Labelled tumour voxels must be replaced even when their current intensity
    # is exactly zero. Non-tumour zero padding remains untouched.
    scan_noisy[tumour_mask] = noise[tumour_mask]
    return scan_noisy, tumour_mask.astype(np.float32)


def correct_background_zscore(healthy_crop, imgs_recon):
    """Z-score variant: clamp generated values, using 0 as background sentinel.

    Unlike the minmax variant (where background = -1), z-score preprocessed
    data uses 0 for padding/background (matching nnUNet constant_values=0).

    Args:
        healthy_crop: (S,S,S) z-score normalized healthy scan (bg=0)
        imgs_recon:   (S,S,S) diffusion model output

    Returns:
        np.ndarray: corrected (S,S,S) in z-score space
    """
    imgs_corrected = np.copy(imgs_recon).astype(np.float32)
    # Background regions (healthy_crop == 0) → set to 0
    imgs_corrected[healthy_crop == 0] = 0
    return imgs_corrected


def load_diffusion_model(ckpt_path, args, device="cuda", use_compile=False):
    """Load a diffusion model from checkpoint with metadata-aware configuration.

    Loads the checkpoint metadata (noise_embedding_mode, time_ch_count, etc.)
    BEFORE creating the model architecture, ensuring structural compatibility.

    Args:
        ckpt_path:     str, path to diffusion_*.pt checkpoint
        args:          namespace object with generator_type, in_channels, out_channels,
                       feature_size, use_checkpoint, crop_size
        device:        str, "cuda" or "cpu"
        use_compile:   bool, enable torch.compile

    Returns:
        (model, metadata_dict)
        metadata_dict keys: n_steps, noise_schedule, noise_embedding_mode,
                           schedule_config, p_uncond
    """
    ckpt = torch.load(ckpt_path, map_location=torch.device("cpu"))

    # ---- Read architecture metadata BEFORE creating model (critical) ----
    if "noise_embedding_mode" in ckpt:
        args.noise_embedding_mode = ckpt["noise_embedding_mode"]
    if "time_ch_count" in ckpt:
        args.time_ch_count = ckpt["time_ch_count"]
    for field in (
        "generator_type", "in_channels", "out_channels", "feature_size", "crop_size",
        "network_channels", "network_strides",
    ):
        if field in ckpt:
            setattr(args, field, ckpt[field])

    n_steps = ckpt.get("n_steps", 1000)
    model = get_diffusion_network(args, n_steps=n_steps)
    ...
    if use_compile:
        if hasattr(torch, 'compile'):
            ...
            print(f"  Warning: torch.compile not available, skipping")

    metadata = {
        "n_steps": n_steps,
        "noise_schedule": ckpt.get("noise_schedule",
                                     ckpt.get("beta_schedule", "cosine")),
        "noise_embedding_mode": ckpt.get("noise_embedding_mode", "discrete"),
        ...
        "crop_size": ckpt.get("crop_size"),
        "generator_type": ckpt.get("generator_type"),
        "in_channels": ckpt.get("in_channels"),
        "out_channels": ckpt.get("out_channels"),
        "feature_size": ckpt.get("feature_size"),
        "network_channels": ckpt.get("network_channels"),
        "network_strides": ckpt.get("network_strides"),
    }
