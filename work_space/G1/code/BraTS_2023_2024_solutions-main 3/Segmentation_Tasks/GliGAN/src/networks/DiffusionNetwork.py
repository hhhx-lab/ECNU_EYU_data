...
def plan_network_params(input_size=64, gpu_memory_gb=None):
    """
    ...
    # MONAI UNet consumes exactly len(channels) - 1 stride values.
    strides = tuple([2] * (num_stages - 1))
    batch_size = max(1, int(gpu_memory_gb * 0.5))

    return channels, strides, batch_size


def _as_int_tuple(value, field_name):
    if value in (None, ""):
        return None
    ...
    if not result or any(item <= 0 for item in result):
        raise ValueError(f"{field_name} must contain positive integers: {value}")
    return result
...
class TimeConditionedSwinUNETR(nn.Module):
    ...
    def __init__(self, img_size=(96, 96, 96), in_channels=1, cond_channels=3,
                 ...
                 time_emb_dim=128, time_hidden_dim=256, time_ch_count=8,
                 noise_embedding_mode="discrete"):
        super().__init__()
        self.time_ch_count = time_ch_count
        _tmlp_out = time_hidden_dim if noise_embedding_mode == "continuous" else time_emb_dim
        self.time_proj = nn.Linear(_tmlp_out, time_ch_count)
        ...
        backbone_in_ch = in_channels + cond_channels + time_ch_count
    ...
    def forward(self, x, t, cond=None):
        ...
        t_ch = self.time_proj(t_emb)[:, :, None, None, None].expand(-1, -1, x.shape[2], x.shape[3], x.shape[4])
...
class TimeConditionedUNet_NnU(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, cond_channels=3, out_channels=1,
                 input_size=64, gpu_memory_gb=None,
                 network_channels=None, network_strides=None,
                 time_emb_dim=128, time_hidden_dim=256, time_ch_count=8,
                 noise_embedding_mode="discrete"):
        super().__init__()
        self.time_ch_count = time_ch_count
        ...
        channels = _as_int_tuple(network_channels, "network_channels")
        strides = _as_int_tuple(network_strides, "network_strides")
        if channels is None:
            channels, planned_strides, _bs = plan_network_params(input_size, gpu_memory_gb)
            if strides is None:
                strides = planned_strides
        else:
            _bs = max(1, int(gpu_memory_gb * 0.5))
            if strides is None:
                strides = tuple([2] * (len(channels) - 1))
        if len(strides) != len(channels) - 1:
            raise ValueError(
                f"network_strides must have len(network_channels)-1 values: "
                f"channels={channels}, strides={strides}")
        self.network_channels = tuple(channels)
        self.network_strides = tuple(strides)
...
class TimeConditionedPlainConvUNet(nn.Module):
    ...
    def __init__(self, in_channels=1, cond_channels=3, out_channels=1,
        ...
        channels = _as_int_tuple(network_channels, "network_channels")
        strides_monai = _as_int_tuple(network_strides, "network_strides")
        if channels is None:
            channels, planned_strides, _bs = plan_network_params(input_size, gpu_memory_gb)
            if strides_monai is None:
                strides_monai = planned_strides
        else:
            _bs = max(1, int(gpu_memory_gb * 0.5))
            if strides_monai is None:
                strides_monai = tuple([2] * (len(channels) - 1))
        if len(strides_monai) != len(channels) - 1:
            raise ValueError(
                f"network_strides must have len(network_channels)-1 values: "
                f"channels={channels}, strides={strides_monai}")
        self.network_channels = tuple(channels)
        self.network_strides = tuple(strides_monai)
...
class LabelDenoiser3D(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, time_emb_dim=128, time_hidden_dim=256, time_ch_count=8,
        ...
        self.encoder1 = nn.Sequential(
            nn.Conv3d(in_channels + time_ch_count, c, 3, padding=1),
            nn.SiLU(),
        )
...
def get_diffusion_network(args, n_steps=1000):
    ...
    - args.crop_size: spatial size for the crop/pad volume (default 64, glioma use 96)
    """
    ...
    if args.generator_type == "SwinUNETR":
        ...
        model = TimeConditionedUNet_NnU(
            ...
            network_channels=getattr(args, "network_channels", None),
            network_strides=getattr(args, "network_strides", None),
            time_ch_count=time_ch_count,
            noise_embedding_mode=noise_mode,
        )
