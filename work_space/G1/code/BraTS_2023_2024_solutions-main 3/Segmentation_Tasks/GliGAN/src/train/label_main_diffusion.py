...
def create_train_loader(args, CSV_PATH, BATCH_SIZE, WORKERS, LABEL_TRANSFORM, crop_size):
    col_names = ['scan_t1c', 'label', 'center_x', 'center_y', 'center_z',
                 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max',
                 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size', 'n_voxels']
    ...
    train_transforms = Compose([
        ...
        CropLabel(keys=["label"], target_size=crop_size),
        Resized(keys=["label_crop_pad"], spatial_size=(crop_size, crop_size, crop_size)),
...
def train_epoch(model, optimizer, train_loader, schedule_cfg, device):
    model.train()
    total_loss = 0.0
    count = 0
    ...
    for batch in train_loader:
        ...
        if schedule_cfg.name in ("edm",):
            loss = _diffusion_utils.edm_loss_fn(
                model=model, x_0=real_labels, cond=None,
                schedule_cfg=schedule_cfg, device=device)
        elif schedule_cfg.name in ("lognsr",):
            loss = _diffusion_utils.lognsr_loss_fn(
                model=model, x_0=real_labels, cond=None,
                schedule_cfg=schedule_cfg, device=device)
        else:
            # Legacy DDPM: inline unconditional diffusion loss
            ...
            a = schedule_cfg.alphas_bar_sqrt[t].to(device)
            a = a.view(-1, 1, 1, 1, 1)
            aml = schedule_cfg.one_minus_alphas_bar_sqrt[t].to(device).view(-1, 1, 1, 1, 1)
...
def __main__():
    ...
    parser.add_argument("--in_channels", default=None, type=int,
                        help="Input channels (label channels, auto: BRATS_2023=3, BRATS_2024=4). "
    ...
    parser.add_argument("--crop_size", default=64, type=int,
                        help="Crop/pad target size (64=default for label diffusion, 96=glioma)")
    ...
    if args.in_channels is None:
        if args.dataset == "BRATS_2024":
            args.in_channels = 4  # 4 label channels
        elif args.dataset == "BRATS_2024_MENINGIOMA":
            args.in_channels = 1  # 1 label channel
        else:
            args.in_channels = 3  # 3 label channels (BRATS_2023 / GOAT)
        args.out_channels = args.in_channels
        print(f"[Auto] in_channels=out_channels={args.in_channels} (detected from {args.dataset})")
    ...
    # Build noise schedule (legacy beta / EDM / logsnr) BEFORE model creation
    ...
    noise_embedding_mode = "continuous" if schedule_cfg.name in ("edm", "lognsr") else "discrete"
    print(f"[Schedule] {schedule_cfg.name}, embedding_mode={noise_embedding_mode}")

    # Build model
    model = LabelDenoiser3D(in_channels=args.out_channels,
                           noise_embedding_mode=noise_embedding_mode)
    ...
    # Legacy aliases for DDPM/DDIM sampling
