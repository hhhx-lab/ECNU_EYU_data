...
def connected_component_analysis(mask_binary):
    ...
    for cc_id in range(1, n_cc + 1):
        ...
        # nibabel and MONAI expose NIfTI arrays in native (x, y, z) order.
        # These coordinates are consumed directly by the training transforms.
        ...
        components.append({
            'cc_mask': cc_mask,
            'centroid': tuple(round(value) for value in centroid),
    ...
    for tx in xs:
        for ty in ys:
            for tz in zs:
                ...
                # NIfTI arrays are native (x, y, z), matching bbox coordinates.
                tile_n = int(mask_binary[gx_min:gx_max, gy_min:gy_max, gz_min:gz_max].sum())
                if tile_n == 0:
                    continue
...
def load_split_manifest(path):
    """Load the team master split without recomputing patient membership."""
    if not path:
        return {}
    ...
    with open(path, newline="") as f:
        ...
        if not reader.fieldnames or "split" not in reader.fieldnames:
            raise ValueError(f"Split manifest must contain a 'split' column: {path}")
        case_column = next(
            (name for name in ("source_case_id", "case_id") if name in reader.fieldnames),
            None,
        )
        if case_column is None:
            raise ValueError(
                f"Split manifest must contain source_case_id or case_id: {path}")
        ...
        for row in reader:
            ...
            if split not in {"train", "val", "test"}:
                raise ValueError(f"Invalid split '{split}' for {case_id} in {path}")
            if case_id in records and records[case_id]["split"].strip().lower() != split:
                raise ValueError(f"Conflicting split rows for {case_id} in {path}")
            records[case_id] = row
    return records


def validate_case_geometry(case_id, required_paths, label_img):
    """Fail before training if modalities are not voxel-aligned with the label."""
    for modality, path in required_paths.items():
        if modality == "seg":
            continue
        scan_img = nib.load(path)
        if scan_img.shape != label_img.shape:
            raise ValueError(
                f"Geometry mismatch for {case_id}/{modality}: "
                f"shape={scan_img.shape}, seg_shape={label_img.shape}")
        if not np.allclose(scan_img.affine, label_img.affine, rtol=0.0, atol=1e-4):
...
def create_csv(args, DATASET_NAME, CSV_PATH, DATADIR):
    ...
    header = ['patient_id', 'patient_group', 'lesion_id',
              'scan_t1c', 'scan_t2w', 'scan_t2f', 'scan_t1n',
    ...
    split_manifest = load_split_manifest(getattr(args, "split_manifest", ""))
    ...
    for mask_path in training:
            ...
            if ("brats" in DATASET_NAME.lower()) and ("goat" in DATASET_NAME.lower()) and ("2024" in DATASET_NAME.lower()):
                ...
                patient_id = case_id[-6:]
            else:
                raise ValueError("Unknown dataset")

            # Determine split
            if split_manifest:
                manifest_row = split_manifest.get(case_id)
                if manifest_row is None:
                    raise ValueError(f"Case {case_id} is absent from split manifest")
                split = manifest_row["split"].strip().lower()
                if manifest_row.get("t2w_status", "authentic").strip().lower() != "authentic":
                    raise ValueError(f"Case {case_id} is not an authentic-T2W case")
                if split not in {"train", "val"}:
                    skipped_non_train_val += 1
                    print(f"  [SKIP] Case {case_id}: locked split={split}")
                    continue
                patient_group = manifest_row.get("patient_group", "").strip()
            else:
            ...
            if missing:
                ...
                print(f"  [SKIP] Case {case_id}: missing {', '.join(missing)}")
                continue
            ...
            label_values = set(np.unique(mask_data).tolist())
            illegal_labels = label_values - {0.0, 1.0, 2.0, 3.0, 4.0}
            if illegal_labels:
                raise ValueError(f"Illegal segmentation labels in {case_id}: {sorted(illegal_labels)}")
            validate_case_geometry(case_id, required_paths, mask)
            ...
            for idx, cc in enumerate(cc_list):
                ...
                if oversized:
                    ...
                    for ti, tile in enumerate(tiles):
                        ...
                        total_lesions += 1
                        row = [patient_id, patient_group, f"{patient_id}_cc{idx}_t{ti}",
                    ...
                    total_lesions += 1
                    row = [patient_id, patient_group, f"{patient_id}_cc{idx}",
                           scan_path_t1c, scan_path_t2w, scan_path_t2f, scan_path_t1n,
                           label_path, cx, cy, cz,
    ...
    if not rows:
        raise RuntimeError(f"No valid lesion rows were found under {DATADIR}")

    patient_crop_counts = Counter(row[patient_group_idx] for row in rows)
    for row in rows:
        row[patient_n_crops_idx] = patient_crop_counts[row[patient_group_idx]]
    ...
    if skipped_missing_modalities:
        print(f"Skipped {skipped_missing_modalities} case(s) with missing modalities.")
    if skipped_non_train_val:
        print(f"Skipped {skipped_non_train_val} locked test case(s).")
    return valid_cases
...
def __main__():
    ...
    parser.add_argument("--checkpoint_root", default="../../Checkpoint", type=str,
                        help="Root for CSV/debug output when --csv_path is not supplied")
    ...
    parser.add_argument("--split_manifest", default="", type=str,
                        help="Team master CSV with source_case_id/case_id and split. "
                             "When provided, locked test cases are excluded.")
    ...
    if args.csv_path == "":
        CSV_PATH = os.path.join(HOME_DIR, f"{args.logdir}.csv")
    else:
    ...
    if args.debug == "True":
        ...
        if len(df) == 0:
            raise RuntimeError("CSV contains no lesion rows")
