...
def load_ckp(args, model, optimizer, scaler):
    ...
    ckpt = torch.load(
        os.path.join(model_pth, args.modality, "weights", f"diffusion_{args.resume_iter}.pt"),
        map_location="cpu",
    )
    state_dict = {
...
def __main__():
    ...
    parser.add_argument("--checkpoint_root", default="../../Checkpoint", type=str,
                        help="Root directory for checkpoints and training artifacts")
    parser.add_argument("--batch_size", default=16, type=int, help="Batch size")
    parser.add_argument("--num_workers", default=2, type=int,
                        help="Deprecated compatibility option; preprocessing is not cached")
    parser.add_argument("--loader_workers", default=0, type=int,
                        help="DataLoader workers. Keep 0 on the validated server path")
    ...
    parser.add_argument(
        "--network_channels", default="", type=str,
        help="Optional fixed comma-separated channel plan, e.g. 48,96,192,384")
    parser.add_argument(
        "--network_strides", default="", type=str,
        help="Optional fixed comma-separated stride plan, e.g. 2,2,2")
    ...
    parser.add_argument("--modality", default="t1c", choices=["t1c", "t1n", "t2w", "t2f"],
    ...
    parser.add_argument("--split", default="train", choices=["train", "val", "all"],
                        help="CSV split to load. Formal training must use train")
    parser.add_argument("--dataset", type=str, required=True, help="Dataset name")
    parser.add_argument("--normalization", type=str, default="minmax",
                        choices=["minmax", "zscore"],
                        help="Scan normalization: minmax ([-1,1], legacy) or "
                             "zscore (brain-masked ~N(0,1), matching S2 nnUNet preprocessing). "
                             "Use zscore for models serving S2 on-the-fly augmentation.")
    ...
    if args.normalization == "zscore" and abs(args.sigma_data - 1.0) > 1e-8:
        raise ValueError("zscore training requires --sigma_data 1.0")
    ...
    # A planned network must be restored before construction. Otherwise a
    # checkpoint trained on A100 could build a different width on V100/CPU.
    if args.resume_iter is not None:
        resume_path = os.path.join(
            args.checkpoint_root, args.logdir, args.modality, "weights",
            f"diffusion_{args.resume_iter}.pt")
        resume_meta = torch.load(resume_path, map_location="cpu")
        for field in ("network_channels", "network_strides"):
            if resume_meta.get(field):
                setattr(args, field, resume_meta[field])
    ...
    import pandas as pd
    if args.csv_path == "":
        csv_dir = os.path.join(args.checkpoint_root, args.logdir)
        csv_file = None
    ...
    if csv_file and os.path.isfile(csv_file):
        ...
        balance_column = "patient_group" if "patient_group" in df_csv.columns else "patient_id"
        if balance_column not in df_csv.columns:
            raise ValueError(
                f"CSV missing patient_group/patient_id balance column: {csv_file}")
        patient_counts = df_csv.groupby(balance_column).size()
        expected_counts = df_csv[balance_column].map(patient_counts).astype(int)
        needs_update = (
        ...
        if needs_update:
            ...
            print(
                f"[CSV] Rebuilt patient_n_crops by {balance_column} "
                f"({len(patient_counts)} groups)")
    # ---------------------------------------------------------------------------
