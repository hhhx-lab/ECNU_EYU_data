...
python src/train/tumour_main_diffusion.py \
    ...
    --normalization zscore --sigma_data 1.0
```

**旧版（minmax [-1,1] 归一化，仅用于已有 checkpoint 兼容）：**
...
> **重要**：如果扩散模型用于 S2 nnUNet 即时增强，必须用 `--normalization zscore --sigma_data 1.0`。`minmax` 和 `zscore` 两种模式训练的 checkpoint **不兼容**——推理时必须用与训练相同的 `--normalization`。
...
python src/infer/evaluate_generation.py \
    ...
    --generator_type Unet_NnU \
    --crop_size 64 --normalization zscore --sigma_data 1.0 \
    --noise_schedule edm \
...
python src/infer/evaluate_generation.py \
    ...
    --output_dir ./eval_results \
    --generator_type Unet_NnU \
    --crop_size 64 --normalization zscore --sigma_data 1.0 --evaluation_mode whole_brain \
    --noise_schedule edm \
    --sampling_method edm_heun --sampling_steps 18 --use_compile
```
...
|---|---|---|
| `--crop_size` | 64 | 须与训练一致 |
| `--normalization` | `minmax` | 须与训练一致：`minmax` ([-1,1]) / `zscore` (S2 nnUNet) |
...
| `--normalization` | `minmax` | `minmax` ([-1,1]) / `zscore` (brain-masked ~N(0,1), matches S2 nnUNet) |
| `--crop_size` | 64 | crop/pad 目标尺寸 |
...
## 6. 归一化模式与 crop_size 选型

### 归一化：minmax vs zscore

| | `--normalization minmax` | `--normalization zscore` |
|---|---|---|
| **值域** | [-1, 1]（min-max 缩放） | ~N(0,1)（脑掩码 z-score，匹配 S2 nnUNet 预处理） |
| **padding/背景值** | -1 | 0 |
| **sigma_data** | 0.5（默认） | **1.0**（必须） |
| **用途** | 旧版 checkpoint 兼容、独立评估 | **S2 nnUNet 即时增强（推荐）** |
| **即时增强路径** | `rescale_to_mm1()` → 扩散 → `linear_interpolation_vec()` | 扩散直通（z-score 进，z-score 出，无损） |

> 两种模式训练的 checkpoint **不兼容**。`--normalization` 必须在训练和推理时保持一致。
...
python src/train/tumour_main_diffusion.py \
    ...
    --normalization minmax --sigma_data 0.5
...
> 对 minmax 归一化（0~1），0.5 是合理默认值。如果换成 z-score 归一化（`--normalization zscore`），数据 std≈1，建议 `--sigma_data 1.0`。S2 nnUNet 对接必须用 zscore + sigma_data=1.0。
...
**S2 nnUNet 即时增强（z-score，转移瘤，推荐）**：
```bash
--normalization zscore --sigma_data 1.0 --noise_schedule edm \
--sigma_max 50 --sigma_min 0.002 --crop_size 64 \
--small_lesion_weight 3.0 --patient_balance_mode sqrt
```
...
**标准（EDM 论文对齐，minmax）**：
```bash
--normalization minmax --sigma_data 0.5 --noise_schedule edm \
--sigma_max 50 --sigma_min 0.002 --snr_shift 0
```
