...
def diffusion_loss_fn(model, batch_y, batch_x, alphas_bar_sqrt, one_minus_alphas_bar_sqrt,
                      n_steps, device, p_uncond=0.0, sample_weights=None):
    ...
    loss_per_elem = (e - output).square()  # [B, C, D, H, W]
    if sample_weights is not None:
        per_sample = loss_per_elem.mean(dim=[1, 2, 3, 4])  # [B]
        return (sample_weights * per_sample).mean()
    else:
...
def edm_loss_fn(model, x_0, cond, schedule_cfg: NoiseScheduleConfig, device,
                P_mean=-1.2, P_std=1.2, p_uncond=0.0, sample_weights=None):
    """
    ...
    loss_per_elem = weight * (denoised - x_0) ** 2  # [B, C, D, H, W]
    if sample_weights is not None:
        per_sample = loss_per_elem.mean(dim=[1, 2, 3, 4])  # [B]
        loss = (sample_weights * per_sample).mean()
    else:
...
def logsnr_loss_fn(model, x_0, cond, schedule_cfg: NoiseScheduleConfig, device,
                   p_uncond=0.0, sample_weights=None):
    """
    ...
    loss_per_elem = (noise_pred - noise) ** 2  # [B, C, D, H, W]
    if sample_weights is not None:
        per_sample = loss_per_elem.mean(dim=[1, 2, 3, 4])  # [B]
        loss = (sample_weights * per_sample).mean()
    else:
...
def sample_edm(model, shape, cond, schedule_cfg: NoiseScheduleConfig, device,
               ...
               S_noise=1.0, cfg_weight=1.0, known_scan=None, known_mask=None):
    """
    ...
    Args:
        ...
        known_scan:    optional clean known-region image for inpainting
        known_mask:    optional mask (1=preserve known_scan, 0=generate)
    Returns:
    ...
    if num_steps < 2:
        raise ValueError("EDM sampling requires num_steps >= 2")
    if (known_scan is None) != (known_mask is None):
        raise ValueError("known_scan and known_mask must be provided together")
    ...
    if known_scan is not None:
        known_scan = known_scan.to(device=device, dtype=x_cur.dtype)
        known_mask = known_mask.to(device=device, dtype=x_cur.dtype)
        if known_scan.shape != x_cur.shape or known_mask.shape != x_cur.shape:
            raise ValueError(
                f"Inpainting tensors must match sample shape {tuple(x_cur.shape)}; "
                f"got scan={tuple(known_scan.shape)}, mask={tuple(known_mask.shape)}")
        known_noise = torch.randn_like(known_scan)

        def project_known(sample, sigma):
            known_at_sigma = known_scan + sigma * known_noise
            return sample * (1.0 - known_mask) + known_at_sigma * known_mask

        x_cur = project_known(x_cur, t_steps[0])
    else:
        def project_known(sample, sigma):
            return sample
    ...
    for i in range(num_steps):
        ...
        x_cur = project_known(x_cur, sigma_hat)
        ...
        x_next_euler = project_known(x_next_euler, sigma_next)
        ...
        if i < num_steps - 1:
            ...
            x_cur = x_next_euler
        x_cur = project_known(x_cur, sigma_next)
...
def add_noise_schedule_args(parser):
    ...
    g.add_argument("--noise_schedule", default="edm", type=str,
    ...
    g.add_argument("--sigma_max", default=50.0, type=float,
