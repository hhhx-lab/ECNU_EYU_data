...
def process_multiple_subjects(input_subject_list, synthesis_type, output_path,
    ...
    failures = []
    completed = 0
    # Load model weights once and reuse them across every subject in this job.
    device = torch.device(
    ...
    if per_lesion:
        ...
        loaded_models = roi_synthesis.prepare_roi_models(device)
        print("Models loaded.")
    else:
        print("Loading synthesis models once for this job ...")
        loaded_models = pipeline.prepare_synthesis_models(synthesis_type, device)
        print("Models loaded.")
        ...
        try:
            ...
            if per_lesion:
                run_synthesis_per_lesion(s_data, synthesis_type, case_output_path,
                                         out_name, loaded_models, gpu_id=gpu_id,
...
def run_synthesis_per_lesion(s_data, synthesis_type, output_path, output_name,
    ...
    syn_lat_encdec = pipeline.run_encdec_synthesis(
        s_data, device, unet=unet_encdec
    )
    syn_lat_bbdm = pipeline.run_bbdm_synthesis(
        s_data,
        device,
        models=(unet_bbdm, conditions_model, noise_scheduler),
    )
