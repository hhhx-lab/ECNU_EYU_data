...
def instantiate_vae_model(device, weights_path=None):
    parser = ConfigParser(configs.NETWORKS_CONFIG)
    parser.parse(True)

    autoencoder = parser.get_parsed_content("autoencoder_def").to(device)
    checkpoint_path = weights_path or configs.PATH_NAME_WEIGHTS_VAE
    checkpoint_autoencoder = torch.load(checkpoint_path, weights_only=True, map_location=device)
...
def instantiate_encdec_model(device):
    ...
    chk_path = configs.PATH_NAME_WEIGHTS_ENCDEC or utils.get_chkpoint_path(
        configs.PATH_WEIGHTS_ENCDEC, configs.MISSING_MODALITY
    )
    if not os.path.isfile(chk_path):
        raise FileNotFoundError(f"EncDec checkpoint not found: {chk_path}")
    chk = torch.load(chk_path, weights_only=False, map_location=device)
    state_dict = dict(chk["unet_state_dict"])
    if chk.get("ema_state_dict"):
        state_dict.update(chk["ema_state_dict"])
        print(f"Loaded EncDec EMA parameters from: {chk_path}")
...
def instantiate_bbdm_model(device):
    ...
    chk_path = configs.PATH_NAME_WEIGHTS_BBDM or utils.get_chkpoint_path(
        configs.PATH_WEIGHTS_BBDM, configs.MISSING_MODALITY
    )
    if not os.path.isfile(chk_path):
        raise FileNotFoundError(f"BBDM checkpoint not found: {chk_path}")
    chk = torch.load(chk_path, weights_only=False, map_location=device)
    ...
    if chk.get("ema_state_dict"):
        state_dict.update(chk["ema_state_dict"])
        print(f"Loaded BBDM EMA parameters from: {chk_path}")
    unet.load_state_dict(state_dict, strict=True)
...
def prepare_synthesis_models(synthesis_type, device):
    """Load each required model once so a multi-case job can reuse it."""
    if synthesis_type not in ("encdec", "bbdm", "ensamble"):
        raise ValueError(f"Unsupported synthesis type: {synthesis_type}")
    models = {"vae": instantiate_vae_model(device)}
    if synthesis_type in ("encdec", "ensamble"):
        models["unet_encdec"] = instantiate_encdec_model(device)
    if synthesis_type in ("bbdm", "ensamble"):
        (
            models["unet_bbdm"],
            models["conditions_model"],
            models["noise_scheduler"],
        ) = instantiate_bbdm_model(device)
    return models
...
def run_encdec_synthesis(s_data, device, unet=None):
...
def run_bbdm_synthesis(s_data, device, models=None):
    latens_list = s_data["latens_list"]

    if models is None:
        unet, conditions_model, noise_scheduler = instantiate_bbdm_model(device=device)
    else:
        unet, conditions_model, noise_scheduler = models
...
def run_synthesis(
    ...
    # set device
    device = (
        next(models["vae"].parameters()).device
        if models is not None
        else torch.device(f"cuda:{gpu_id}" if gpu_id is not None else "cpu")
    )
    ...
    if synthesis_type in ("encdec", "ensamble"):
        ...
        syn_latens_encdec = run_encdec_synthesis(
            s_data,
            device,
            unet=models.get("unet_encdec") if models is not None else None,
        )
    if synthesis_type in ("bbdm", "ensamble"):
        if verbose and synthesis_type == "ensamble":
        ...
        syn_latens_bbdm = run_bbdm_synthesis(s_data, device, models=bbdm_models)
