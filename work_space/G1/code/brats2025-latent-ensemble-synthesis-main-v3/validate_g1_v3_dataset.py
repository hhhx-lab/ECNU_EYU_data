#!/usr/bin/env python3
"""Fail-fast validation for the G1 V3 training CSV and linked NIfTI files."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("data/g1_v3_dataset_validation.json"),
    )
    parser.add_argument(
        "--skip-label-values",
        action="store_true",
...
def main() -> None:
    ...
    if errors:
        ...
        raise RuntimeError(
            f"G1 V3 dataset validation failed with {len(errors)} error(s); see {args.output_json}."
        )
