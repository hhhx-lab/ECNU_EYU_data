#!/usr/bin/env python3
"""Validate G1 V3 placement, fake-T2W isolation, and internal split counts."""
...
def main() -> None:
    ...
    placement = read_csv(data_dir / "g1_v3_data_placement_manifest.csv")
    data_rows = read_csv(data_dir / "data_csv.csv")
    label_rows = read_csv(data_dir / "g1_v3_label_filter_report.csv")
    ...
    output = data_dir / "g1_v3_preparation_validation.json"
    output.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n")
    print(json.dumps(summary, indent=2, ensure_ascii=False))
    if errors:
        raise RuntimeError(f"G1 V3 preparation validation failed; see {output}")
