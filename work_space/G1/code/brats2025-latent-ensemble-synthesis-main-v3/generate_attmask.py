...
Output:
    data/attention_masks/<subject_id>/<subject_id>_attmask_256_256_160.npy
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        # Save both image-space and latent-space masks. Training uses the latent
        # mask; validation visualizations use the image-space mask.
        out_dir = os.path.join(attmask_dir, s_id)
        os.makedirs(out_dir, exist_ok=True)
        image_tag = "_".join(str(s) for s in image_shape)
        image_out_path = os.path.join(
            out_dir, f"{s_id}_attmask_{image_tag}.npy"
        )
        np.save(image_out_path, seg)
    ...
    if missing_seg:
        raise RuntimeError(
            f"Attention-mask generation missed {len(missing_seg)} cases: {missing_seg[:10]}"
        )
    if processed != len(df):
        raise RuntimeError(
            f"Attention-mask count mismatch: processed={processed}, expected={len(df)}"
        )
    print(f"\nDone. Generated image/latent masks for {processed} cases.")
