...
### Step 2 — Evaluate on the fixed validation split
...
# Saved:    data/eval_synthesized_val/<subject_id>-t2w.nii.gz
```

For model selection and tuning, do not evaluate all of `data/input` together. Use the fixed `val` split from `data_csv.csv`. If validation metrics and images look good, proceed. Otherwise tune hyperparameters and retrain.
...
| `work_space/G1/data/eval_synthesized_val/` | `evaluate.py --split val --save_output` | Validation-split synthesized images for inspection |
...
| `--compute_bmask` | False | Segment and apply brain mask. Requires TotalSegmentator; do not enable in the server Slurm unless that dependency is installed and checked. |
...
│   └── eval_synthesized_val/          # Validation results (evaluate.py --split val --save_output)
├── training/
