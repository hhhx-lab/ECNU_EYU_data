...
class DiffusionModelUNetMaisi(nn.Module):
    ...
    def __init__(
        ...
        include_from_modality: bool = False,
        include_to_modality: bool = False,
        ...
        self.include_from_modality = include_from_modality
        self.include_to_modality = include_to_modality

        new_time_embed_dim = time_embed_dim

        if self.include_from_modality:
            self.from_modality_layer = self._create_embedding_module(4, time_embed_dim)
            new_time_embed_dim += time_embed_dim
        if self.include_to_modality:
            self.to_modality_layer = self._create_embedding_module(4, time_embed_dim)
            new_time_embed_dim += time_embed_dim
    ...
    def _get_input_embeddings(self, emb, from_modality_tensor, to_modality_tensor):
        if self.include_from_modality:
            _emb = self.from_modality_layer(from_modality_tensor)
            emb = torch.cat((emb, _emb), dim=1)
        if self.include_to_modality:
            _emb = self.to_modality_layer(to_modality_tensor)
            emb = torch.cat((emb, _emb), dim=1)
        return emb
    ...
    def forward(
        ...
        from_modality_tensor: torch.Tensor | None = None,
        to_modality_tensor: torch.Tensor | None = None,
        ...
        # REMOVED SPACING AND OTHER LAGACY CODE
        ...
        emb = self._get_time_and_class_embedding(x, timesteps) # class labels no se usa
        emb = self._get_input_embeddings(emb, from_modality_tensor, to_modality_tensor)
        h = self.conv_in(x)
