...
def make_diffusion_coefficients(n_steps=1000, beta_schedule="cosine", device="cpu"):
    """Build beta / alpha coefficients needed for DDPM / DDIM sampling.

    Returns (betas, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, alphas_bar).
    alphas_bar (non-sqrt) is required by DDIM and computed here for convenience.
    """
    betas = _diffusion_utils.make_beta_schedule(
        schedule=beta_schedule, num_timesteps=n_steps
    )
    alphas = 1.0 - betas
    alphas_bar = torch.cumprod(alphas, dim=0)
    alphas_bar_sqrt = torch.sqrt(alphas_bar).to(device)
    one_minus_alphas_bar_sqrt = torch.sqrt(1.0 - alphas_bar).to(device)
    return betas.to(device), alphas_bar_sqrt, one_minus_alphas_bar_sqrt, alphas_bar.to(device)
...
def load_diffusion_model(args, model_path, network_type="tumour"):
    ...
    model.eval()
    return model, n_steps, beta_schedule
