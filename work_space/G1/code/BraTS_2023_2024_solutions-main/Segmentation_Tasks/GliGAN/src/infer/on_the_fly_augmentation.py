...
class OnTheFlyTumourAugmenter:
    ...
    def augment_sample(self, data_4ch, seg, rng=None):
        ...
        输入:
            data_4ch: np.ndarray, 形状 (4, D, H, W)
                      4 个 MRI 模态 (t1ce, t1, t2, flair)，z-score 归一化
    ...
    def _insert_one_tumour(self, data_4ch, seg, tumour_label_96, meta, rng):
        ...
        modality_keys = ["t1ce", "t1", "t2", "flair"]
...
def __main__():
    ...
    if args.test_case:
        ...
        # 假设 4 模态：目录下通常有 t1ce, t1, t2, flair .nii.gz 文件
        test_dir = os.path.dirname(args.test_case)
        test_basename = os.path.basename(args.test_case).replace("-t1ce.nii.gz", "")
        modalities = ["t1ce", "t1", "t2", "flair"]
        data_4ch = []
        for mod in modalities:
            ...
            if os.path.exists(mod_path):
                ...
                print(f"  [警告] 找不到 {mod_path}，用 t1ce 代替")
