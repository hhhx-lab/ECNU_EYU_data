...
and saves the results under a per-case output folder as nii.gz files.
...
def load_label_and_prepare(label_path, dataset_type, target_size=96):
    ...
    Returns:
        ...
        label_single_96:  (target_size, target_size, target_size) int16
    """
    ...
    if dataset_type in {"BRATS_2024", "BRATS_2026"}:
        n_channels = 4
    ...
    else:  # Legacy BRATS_2023 / BRATS_GOAT_2024 support only.
        n_channels = 3
        label_mc = np.zeros((n_channels,) + label_data.shape, dtype=np.float32)
        label_mc[0] = ((label_data == 1) | (label_data == 3)).astype(np.float32)  # TC
        label_mc[1] = ((label_data == 1) | (label_data == 2) | (label_data == 3)).astype(np.float32)  # WT
    ...
    crop_label = label_data[z0:z1, y0:y1, x0:x1]
    ...
    if max_dim > target_size - 4:
        ...
        for c in range(n_channels):
            resized[c] = ndimage_zoom(crop_mc[c].astype(np.float32),
                                       ...
                                       order=1)
        crop_label = ndimage_zoom(
            crop_label.astype(np.float32),
            tuple(new_dims / np.array(crop_mc.shape[1:])),
            order=0,
        ).astype(np.int16)
        crop_mc = resized
    ...
    label_single_96 = np.zeros((target_size, target_size, target_size), dtype=np.int16)
    label_single_96[
        z_start:z_start + crop_label.shape[0],
        y_start:y_start + crop_label.shape[1],
        x_start:x_start + crop_label.shape[2],
    ] = crop_label.astype(np.int16)

    meta = {"z0": z0, "z1": z1, "y0": y0, "y1": y1, "x0": x0, "x1": x1,
            "original_shape": label_data.shape, "n_channels": n_channels}

    return result, meta, label_single_96
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="BRATS_2026",
                        help="Dataset type: BRATS_2023 / BRATS_2024 / BRATS_2026 / BRATS_GOAT_2024")
    ...
    if args.dataset == "BRATS_2026" and "BraTS-MET-" not in os.path.basename(args.label_path):
        raise ValueError(f"BRATS_2026 generation only accepts BraTS-MET-* labels: {args.label_path}")
    ...
    if args.dataset in {"BRATS_2024", "BRATS_2026"}:
        label_channels = 4
    else:
        label_channels = 3
    ...
    label_cube, meta, label_single_96 = load_label_and_prepare(args.label_path, args.dataset, target_size=96)
    ...
    case_output_dir = os.path.join(args.output_dir, basename)
    shutil.rmtree(case_output_dir, ignore_errors=True)
    os.makedirs(case_output_dir, exist_ok=True)
    seg_output_path = os.path.join(case_output_dir, f"{basename}-seg.nii.gz")
    nib.save(nib.Nifti1Image(label_single_96.astype(np.int16), np.eye(4)), seg_output_path)
    ...
    for mod in modalities:
        ...
        output_path = os.path.join(case_output_dir, f"{basename}-{mod}.nii.gz")
