...
Usage:
    ...
    python evaluate_generation.py \
        ...
        --csv_path ../../Checkpoint/brats2026_diffusion/brats2026_diffusion.csv \
        --dataset BRATS_2026 \
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="BRATS_2026",
                        choices=["BRATS_2023", "BRATS_2024", "BRATS_2026", "BRATS_GOAT_2024"])
    ...
    label_channels = 4 if args.dataset in {"BRATS_2024", "BRATS_2026"} else 3
    ...
    if args.dataset == "BRATS_2026" and "id" in df.columns:
        ...
        if non_met:
            ...
            raise ValueError(f"BRATS_2026 evaluation refuses non-MET case ids in CSV: {preview}")
    if args.max_cases > 0:
    ...
    for mod in modalities:
        ...
        for idx, row in df.iterrows():
            ...
            # CSV now stores absolute source paths from the 2026 Task1 raw tree.
            scan_full = scan_path if os.path.isabs(scan_path) else os.path.abspath(scan_path)
            label_full = label_path if os.path.isabs(label_path) else os.path.abspath(label_path)
