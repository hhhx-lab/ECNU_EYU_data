...
# GAN-based label training has been replaced by diffusion.
# See: src/train/label_main_diffusion.py
#      src/networks/DiffusionNetwork.py (LabelDenoiser3D)
# ============================================================
...
from monai.transforms import Compose, ScaleIntensity, ToTensor, Resize, RandRotate, RandFlip, RandScaleIntensity, RandZoom, RandGaussianNoise, RandAffine, ResizeWithPadOrCrop, EnsureChannelFirst
from monai.data import CacheDataset, ImageDataset
from monai.data.utils import pad_list_data_collate
from monai.data import CSVDataset
from monai.transforms import (
    Compose, LoadImaged, EnsureChannelFirstd, EnsureTyped,
    ScaleIntensityd, CopyItemsd, CropForegroundd, SpatialCropd,
...
def create_train_loader(args, CSV_PATH, BATCH_SIZE, WORKERS, LABEL_TRANSFORM):
    ...
    warnings.warn("The data loader will load all labels to memory. In case it fails due to lack of memory, reduce the 'cache_rate'.")
...
def see_information(iteration, loss2, loss1, loss3, mse_loss, gd_loss, Lloss1, Lloss2, Lloss3, MSELossL, Gd_LossL, TOTAL_ITER):
...
def create_dirs(HOME_DIR):
    for subdir in ["", "/label", "/label/weights", "/label/loss_lists", "/label/checkpoint_scans"]:
        if not os.path.exists(HOME_DIR+subdir):
            os.makedirs(HOME_DIR+subdir)
    print("## ALL dirs set ##")
...
def training(args, resume, start, HOME_DIR, TOTAL_ITER, LATENT_DIM, train_loader,
             CD_Z_HAT_WEIGHT, D_WEIGHT, MSE_WEIGHT, GD_WEIGHT, X_LOSS2_WEIGHT,
             GP_D_WEIGHT, X_LOSS3_WEIGHT, GP_CD_WEIGHT, _EPS):
    ...
    if resume:
        G,CD,D,E,g_optimizer,cd_optimizer,d_optimizer,e_optimizer = load_dict(
            HOME_DIR,start,G,CD,D,E,g_optimizer,cd_optimizer,d_optimizer,e_optimizer)
    ...
    for iteration in range(start,TOTAL_ITER+1):
        ...
        Lloss1,Lloss2,Lloss3,MSELossL,Gd_LossL=see_information(
            iteration=iteration, loss2=loss2, loss1=loss1, loss3=loss3,
            mse_loss=mse_loss, gd_loss=gd_loss, Lloss1=Lloss1, Lloss2=Lloss2,
            Lloss3=Lloss3, MSELossL=MSELossL, Gd_LossL=Gd_LossL, TOTAL_ITER=TOTAL_ITER)

        if (iteration)%1000 ==0:
            checkpoint_G = {"iteration": iteration, "state_dict": G.state_dict(), "g_optimizer": g_optimizer.state_dict()}
            save_ckp(checkpoint_G, f"{HOME_DIR}/label/weights/G_iter_{iteration}.pt")
            checkpoint_D = {"iteration": iteration, "state_dict": D.state_dict(), "d_optimizer": d_optimizer.state_dict()}
            save_ckp(checkpoint_D, f"{HOME_DIR}/label/weights/D_iter_{iteration}.pt")
            checkpoint_E = {"iteration": iteration, "state_dict": E.state_dict(), "e_optimizer": e_optimizer.state_dict()}
            save_ckp(checkpoint_E, f"{HOME_DIR}/label/weights/E_iter_{iteration}.pt")
            checkpoint_CD = {"iteration": iteration, "state_dict": CD.state_dict(), "cd_optimizer": cd_optimizer.state_dict()}
...
def __main__():
    ...
    parser.add_argument("--cd_z_hat_weight", default=1, type=int)
    parser.add_argument("--d_weight", default=1, type=float)
    parser.add_argument("--mse_weight", default=100, type=float)
    parser.add_argument("--gd_weight", default=1/100, type=float)
    parser.add_argument("--x_loss2_weight", default=1, type=float)
    parser.add_argument("--gp_d_weight", default=100, type=float)
    parser.add_argument("--x_loss3_weight", default=1, type=float)
    parser.add_argument("--gp_cd_weight", default=100, type=float)
    ...
    train_loader = create_train_loader(args=args, CSV_PATH=CSV_PATH, BATCH_SIZE=BATCH_SIZE, WORKERS=WORKERS, LABEL_TRANSFORM=LABEL_TRANSFORM)
    ...
    training(args=args, resume=RESUME, start=START, HOME_DIR=HOME_DIR, TOTAL_ITER=TOTAL_ITER, LATENT_DIM=LATENT_DIM,
             train_loader=train_loader, CD_Z_HAT_WEIGHT=CD_Z_HAT_WEIGHT, D_WEIGHT=D_WEIGHT,
             MSE_WEIGHT=MSE_WEIGHT, GD_WEIGHT=GD_WEIGHT, X_LOSS2_WEIGHT=X_LOSS2_WEIGHT,
             GP_D_WEIGHT=GP_D_WEIGHT, X_LOSS3_WEIGHT=X_LOSS3_WEIGHT, GP_CD_WEIGHT=GP_CD_WEIGHT, _EPS=_EPS)
