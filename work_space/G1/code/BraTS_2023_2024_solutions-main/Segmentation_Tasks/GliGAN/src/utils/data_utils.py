...
def get_loader(args): 
    ...
    if args.csv_path == "":
        checkpoint_dir = f"../../Checkpoint/{args.logdir}"
        preferred_csv = os.path.join(checkpoint_dir, f"{args.logdir}.csv")
        if os.path.exists(preferred_csv):
            ...
            csv_candidates = sorted(
                ...
                if file_name.endswith(".csv") and not file_name.endswith("_skipped.csv")
            )
            if not csv_candidates:
                raise FileNotFoundError(f"No training CSV found in {checkpoint_dir}")
            CSV_PATH = csv_candidates[0]
    else:
        CSV_PATH = args.csv_path
    print(f"CSV_PATH: {CSV_PATH}")
    
    if args.modality == "t1c":
        scan_name = "scan_t1c"
        col_names = ['scan_t1c', 'label', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']
    ...
    elif args.modality == "t1n":
        scan_name = "scan_t1n"
        col_names = ['scan_t1n', 'label', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']
        col_types= {'center_x': {'type': int}, 'center_y': {'type': int}, 'center_z': {'type': int}, 'x_extreme_min': {'type': int}, 'x_extreme_max': {'type': int}, 'y_extreme_min': {'type': int}, 'y_extreme_max': {'type': int}, 'z_extreme_min': {'type': int}, 'z_extreme_max': {'type': int}, 'x_size': {'type': int}, 'y_size': {'type': int}, 'z_size': {'type': int}}
    elif args.modality == "t2w":
        scan_name = "scan_t2w"
        col_names = ['scan_t2w', 'label', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']
        col_types= {'center_x': {'type': int}, 'center_y': {'type': int}, 'center_z': {'type': int}, 'x_extreme_min': {'type': int}, 'x_extreme_max': {'type': int}, 'y_extreme_min': {'type': int}, 'y_extreme_max': {'type': int}, 'z_extreme_min': {'type': int}, 'z_extreme_max': {'type': int}, 'x_size': {'type': int}, 'y_size': {'type': int}, 'z_size': {'type': int}}
    elif args.modality == "t2f":
        scan_name = "scan_t2f"
        col_names = ['scan_t2f', 'label', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']
        col_types= {'center_x': {'type': int}, 'center_y': {'type': int}, 'center_z': {'type': int}, 'x_extreme_min': {'type': int}, 'x_extreme_max': {'type': int}, 'y_extreme_min': {'type': int}, 'y_extreme_max': {'type': int}, 'z_extreme_min': {'type': int}, 'z_extreme_max': {'type': int}, 'x_size': {'type': int}, 'y_size': {'type': int}, 'z_size': {'type': int}}    
    ...
    if args.dataset=="BRATS_2023" or args.dataset=="BRATS_GOAT_2024":
    ...
    elif args.dataset=="BRATS_2024" or args.dataset=="BRATS_2026":
        ...
        raise ValueError("The dataset must be from BraTS: BRATS_GOAT_2024, BRATS_2026, BRATS_2024, BRATS_2023 or BRATS_2024_MENINGIOMA")
    ...
    if args.noise_type=="gaussian_extended":
        ...
        train_transforms = Compose(
                    [
                        ...
                        GaussianNoiseTumourExtended(keys=scan_name, normalization=normalization),
                        ToTensord(keys=[scan_name, f'{scan_name}_crop', f'{scan_name}_crop_pad', f'{scan_name}_noisy', 'label', 'label_crop_pad', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']),
                    ]
                )
        ...
        train_transforms = Compose(
                    [
                        ...
                        #),
                        LABEL_TRANSFORM(keys="label"),
                        GaussianNoiseTumour(keys=scan_name, normalization=normalization),
                        ToTensord(keys=[scan_name, f'{scan_name}_crop', f'{scan_name}_crop_pad', f'{scan_name}_noisy', 'label', 'label_crop_pad', 'center_x', 'center_y', 'center_z', 'x_extreme_min', 'x_extreme_max', 'y_extreme_min', 'y_extreme_max', 'z_extreme_min', 'z_extreme_max', 'x_size', 'y_size', 'z_size']),
                    ]
                )
