...
def train(args, global_step, train_loader, generator, G_optimizer, recon_loss, HOME_DIR):
    ...
    for step, batch in enumerate(train_loader):
        ...
        x_crop_pad = batch["scan_t1c_crop_pad"].cuda() # Scan cropped and with padding to 96x96x96
        scan_t1c_noisy = batch["scan_t1c_noisy"].cuda() # Scan cropped, with padding to 96x96x96 and noise
        ...
        # Compute L1 loss
        input_noise = torch.cat([scan_t1c_noisy, y_crop_pad], dim=1) 
    ...
    return global_step, generator, G_optimizer, loss_gen_list, loss_recons_list, geral_generator, x_crop_pad, y_crop_pad, scan_t1c_noisy, scan_recon
...
def __main__():
    ...
    parser.add_argument("--modality", default="t1c", type=str, help="What modality to train: t1n, t2w, t1c, t2f")
    ...
    while global_step < args.num_steps:
        # Start training!
        epoch+=1
        global_step, generator, G_optimizer, loss_gen_list, loss_recons_list, geral_generator, x_crop_pad, y_crop_pad, scan_t1c_noisy, scan_recon = train(
        ...
        save_sample(args=args, image=scan_t1c_noisy[0], reality="scan_t1c_noisy", iter_num=global_step, path=HOME_DIR)
