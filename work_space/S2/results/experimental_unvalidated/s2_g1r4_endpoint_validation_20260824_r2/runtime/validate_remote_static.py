...
def main() -> None:
    ...
    provenance = plan["g1_runtime_provenance"]
    require(provenance["mode"] == "bundled_historical_r4_runtime_snapshot", "G1 runtime mode drift")
    require(provenance["shared_runtime_used_directly"] is False, "shared G1 runtime unexpectedly enabled")
    g1_code = Path(provenance["bundled_remote_root"])
    require(g1_code == root / "runtime/g1_r4_frozen", "bundled G1 runtime root drift")
    ...
    import_env.update(
        {
            ...
            "PYTHONPATH": str(g1_code),
            "G1_VAE_WEIGHTS": plan["g1_r4_checkpoints"]["vae"]["remote_path"],
            "G1_ENCDEC_CHECKPOINT": plan["g1_r4_checkpoints"]["encdec"]["remote_path"],
            "G1_BBDM_CHECKPOINT": plan["g1_r4_checkpoints"]["bbdm"]["remote_path"],
            "G1_BBDM_S": "0.01",
        }
    )
    import_probe = subprocess.run(
        [
            ...
            "-c",
            (
                "import configs,json,synthesis.pipeline as p;"
                "print(json.dumps({'configs':configs.__file__,'pipeline':p.__file__,"
                "'vae':configs.PATH_NAME_WEIGHTS_VAE,'encdec':configs.PATH_NAME_WEIGHTS_ENCDEC,"
                "'bbdm':configs.PATH_NAME_WEIGHTS_BBDM,'bbdm_s':configs.BBDM_S},sort_keys=True))"
            ),
        ],
        env=import_env,
        text=True,
    ...
    require(import_probe.returncode == 0, f"bundled G1 import probe failed: {import_probe.stdout}")
    import_lines = [line for line in import_probe.stdout.splitlines() if line.strip()]
    require(bool(import_lines), "bundled G1 import probe produced no output")
    g1_import_binding = json.loads(import_lines[-1])
    require(Path(g1_import_binding["configs"]).is_relative_to(g1_code), "configs imported outside bundle")
    require(Path(g1_import_binding["pipeline"]).is_relative_to(g1_code), "pipeline imported outside bundle")
    require(g1_import_binding["bbdm_s"] == 0.01, "imported BBDM s drift")
    for name in ("vae", "encdec", "bbdm"):
        require(
            Path(g1_import_binding[name]) == Path(plan["g1_r4_checkpoints"][name]["remote_path"]),
            f"imported G1 {name} checkpoint binding drift",
        )
    ...
    payload = {
        ...
        "g1_runtime_mode": provenance["mode"],
        "shared_g1_runtime_used_directly": False,
        "g1_import_binding": g1_import_binding,
