...
def prepare_smoke_manifest(
    *,
    ...
    allowed_target_groups: set[str] | None = None,
    allowed_donor_groups: set[str] | None = None,
    smoke_id_prefix: str = "route-a-smoke",
) -> dict[str, Any]:
    ...
    if not smoke_id_prefix or any(character.isspace() for character in smoke_id_prefix):
        raise MetAugContractError("Gate 2 smoke ID prefix is invalid")
    expected_targets = {
        case_id
        for case_id, group in manifest.target_groups.items()
        if allowed_target_groups is None or group in allowed_target_groups
    }
    if set(assets) != expected_targets:
        raise MetAugContractError(
            "Gate 2 assets and allowed target IDs differ"
        )
    if not expected_targets:
        raise MetAugContractError("Gate 2 target scope is empty")

    eligible_records = tuple(
        record
        for record in config.eligible_records(manifest)
        if allowed_donor_groups is None or record.patient_group in allowed_donor_groups
    )
    ...
    for candidate_index in range(int(max_candidates)):
        ...
        if allowed_donor_groups is not None and record.patient_group not in allowed_donor_groups:
            skipped["donor_outside_allowed_scope"] += 1
            continue
        ...
        entry = {
            "smoke_id": f"{smoke_id_prefix}-{len(entries) + 1:03d}",
    ...
    if config.fix_v2 is not None:
        payload["fix_v2"] = config.fix_v2.as_mapping()
    if allowed_target_groups is not None or allowed_donor_groups is not None:
        payload["selection_scope"] = {
            "allowed_target_groups": sorted(allowed_target_groups or ()),
            "allowed_donor_groups": sorted(allowed_donor_groups or ()),
            "target_case_count": len(expected_targets),
            "donor_component_count": len(eligible_records),
        }
...
def load_smoke_manifest(
    ...
    *,
    ...
    if payload.get("fix_v2") != expected_fix_v2:
        raise MetAugContractError("Gate 2 smoke manifest Fix-v2 policy drifted")
    selection_scope = payload.get("selection_scope")
    allowed_target_groups: set[str] | None = None
    allowed_donor_groups: set[str] | None = None
    if selection_scope is not None:
        if not isinstance(selection_scope, dict):
            raise MetAugContractError("Gate 2 smoke manifest selection scope is malformed")
        allowed_target_groups = {
            str(value) for value in selection_scope.get("allowed_target_groups", ())
        }
        allowed_donor_groups = {
            str(value) for value in selection_scope.get("allowed_donor_groups", ())
        }
        if not allowed_target_groups or not allowed_donor_groups:
            raise MetAugContractError("Gate 2 smoke manifest selection scope is empty")
        expected_target_ids = {
            case_id
            for case_id, group in manifest.target_groups.items()
            if group in allowed_target_groups
        }
        if int(selection_scope.get("target_case_count", -1)) != len(expected_target_ids):
            raise MetAugContractError("Gate 2 smoke manifest target scope count drifted")
    else:
        expected_target_ids = set(manifest.target_groups)
    eligible_records = tuple(
        record
        for record in config.eligible_records(manifest)
        if allowed_donor_groups is None or record.patient_group in allowed_donor_groups
    )
    if selection_scope is not None and int(
        selection_scope.get("donor_component_count", -1)
    ) != len(eligible_records):
        raise MetAugContractError("Gate 2 smoke manifest donor scope count drifted")
    ...
    for entry in cases:
        ...
        if target_case_id not in expected_target_ids or target_case_id in target_ids:
        ...
        if allowed_donor_groups is not None and record.patient_group not in allowed_donor_groups:
            raise MetAugContractError("Gate 2 smoke donor lies outside the selection scope")
        if not config.is_record_eligible(record):
            raise MetAugContractError(
...
def validate_automatic_report(
    ...
    *,
    ...
    expected = {
        ...
        "fix_v2": smoke_manifest.get("fix_v2"),
    }
    for key, value in expected.items():
        if payload.get(key) != value:
