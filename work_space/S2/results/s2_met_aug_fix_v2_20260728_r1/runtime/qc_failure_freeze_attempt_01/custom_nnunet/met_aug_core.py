...
class MetAugEngine:
    ...
    def __init__(
        ...
        *,
        ...
        if config.fix_v2 is None and candidate_processor is not None:
            raise MetAugContractError(
                "legacy Route A config cannot use a Fix-v2 candidate processor"
            )
        if config.fix_v2 is not None and candidate_processor is not None:
            if candidate_processor.boundary_policy != config.fix_v2.boundary_policy:
                raise MetAugContractError(
                    "Fix-v2 processor boundary policy does not match route config"
                )
            if candidate_processor.calibration_sha256 != config.fix_v2.calibration_sha256:
                raise MetAugContractError(
                    "Fix-v2 processor calibration does not match route config"
                )
    ...
    def apply(
        ...
        *,
        ...
        if self.config.fix_v2 is not None:
    ...
    def _apply_fix_v2(
        ...
        *,
        ...
        if self.candidate_processor is None:
            raise MetAugContractError(
                "Fix-v2 Route A requires a frozen candidate processor"
            )
        if self.backend is None:
            raise MetAugContractError("Fix-v2 Route A requires a diffusion backend")
        assert result.placement is not None
        ...
        fix_v2_metadata = {"fix_v2": dict(processed.metadata)}
        if processed.reason is not None:
            rejected = EventResult(
                ...
                record=result.record,
                placement=placement,
                metadata={**result.metadata, **fix_v2_metadata},
        ...
        if (
            ...
            rejected = EventResult(
                ...
                metadata={
                    **result.metadata,
                    **fix_v2_metadata,
                    "detail": "Fix-v2 processor returned malformed candidate arrays",
                },
            )
            self._append(rejected)
            return image, segmentation, rejected
        ...
        if (
            ...
            rejected = EventResult(
                ...
                metadata={
                    **result.metadata,
                    **fix_v2_metadata,
                    "detail": "Fix-v2 support masks violate the placement contract",
                },
            )
            self._append(rejected)
            return image, segmentation, rejected
