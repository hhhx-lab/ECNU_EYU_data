...
EVAL_ROOT="${EVAL_ROOT:-/root/brats2026/runs/s2_b_baseline_true1mm_eval_20260727_r1}"
...
B_CHECKPOINT="${B_CHECKPOINT:-${PROJECT_ROOT}/work_space/S2/results/s2_completion_dataset264_t2w_20260720/fold_0/checkpoint_final.pth}"
EXPECTED_B_SHA256="78eccc59f9217a529cafdd522733de9a1578f0e96d8765ee7c48731027824db5"
...
EXPECTED_CUDA_VISIBLE_DEVICES="${S2_FROZEN_B_EVAL_GPU:-1}"
...
export EVAL_ROOT FALLBACK_ROOT OVERLAY_ROOT OVERLAY_AUDIT PLANS_FILE SPLIT_DIR B_CHECKPOINT
export EXPECTED_B_SHA256 EXPECTED_TRAIN_SHA256 EXPECTED_VAL_SHA256 EXPECTED_PLANS_SHA256
...
[[ "${CUDA_VISIBLE_DEVICES:-}" == "${EXPECTED_CUDA_VISIBLE_DEVICES}" ]] || {
    echo "Frozen-B evaluation must be bound to physical GPU ${EXPECTED_CUDA_VISIBLE_DEVICES}" >&2
    exit 1
}
...
s2_uhost_require_file "${B_CHECKPOINT}" "frozen B checkpoint"

[[ "$(s2_uhost_sha256 "${B_CHECKPOINT}")" == "${EXPECTED_B_SHA256}" ]] || { echo "Frozen B SHA256 drifted" >&2; exit 1; }
...
if pgrep -af '[n]nUNetv2_train.*nnUNetTrainerBraTS2026RCCompletionFineTune.*--val' >/dev/null; then
    echo "A frozen-B validation process is already active" >&2
    exit 1
fi
if [[ -e "${EVAL_ROOT}" ]]; then
    echo "Frozen-B evaluation root is immutable and already exists: ${EVAL_ROOT}" >&2
    exit 1
fi
...
checkpoint = torch.load(os.environ["B_CHECKPOINT"], map_location="cpu", weights_only=False)
if checkpoint.get("trainer_name") != os.environ["TRAINER"]:
    raise SystemExit("frozen B checkpoint trainer identity drifted")
if checkpoint.get("current_epoch") != 200:
    raise SystemExit("frozen B checkpoint is not the completed epoch-200 artifact")
init_args = checkpoint.get("init_args", {})
if init_args.get("configuration") != "3d_fullres" or init_args.get("fold") != 0:
    raise SystemExit("frozen B checkpoint configuration/fold drifted")
if init_args.get("plans") != plans:
    raise SystemExit("frozen B checkpoint embedded plans differ from true-1mm plans")
...
for name in runtime_files:
    ...
    if not source.is_file() or not deployed.is_file() or source.read_bytes() != deployed.read_bytes():
        raise SystemExit(f"deployed frozen-B trainer runtime drifted: {name}")
    runtime_sha[name] = sha256(source)

payload = {
    "schema_version": 1,
    "experiment": "S2-frozen-B-true1mm-fixed103-evaluation",
    "evaluation_root": os.environ["EVAL_ROOT"],
    "dataset": "Dataset264_BraTS2026_MET_Completion",
    "configuration": "3d_fullres",
    "fold": 0,
    "trainer": os.environ["TRAINER"],
    "checkpoint": os.environ["B_CHECKPOINT"],
    "checkpoint_sha256": os.environ["EXPECTED_B_SHA256"],
...
result_fold = Path(os.environ["RESULTS_ROOT"]) / "Dataset264_BraTS2026_MET_Completion" / f"{os.environ['TRAINER']}__nnUNetPlans__3d_fullres" / "fold_0"
result_fold.mkdir(parents=True)
(result_fold / "checkpoint_final.pth").symlink_to(Path(os.environ["B_CHECKPOINT"]))
...
cd "${S2_REPOSITORY}"
echo "S2_FROZEN_B_EVAL_START gpu=${CUDA_VISIBLE_DEVICES:-unset} validation_count=103"
...
s2_uhost_require_file "${VALIDATION_DIR}/summary.json" "frozen-B validation summary"
...
[[ "${NIFTI_COUNT}" == "103" ]] || { echo "Incomplete frozen-B NIfTI predictions: ${NIFTI_COUNT}/103" >&2; exit 1; }
[[ "${NPZ_COUNT}" == "103" ]] || { echo "Incomplete frozen-B NPZ predictions: ${NPZ_COUNT}/103" >&2; exit 1; }
[[ "$(s2_uhost_sha256 "${RESULT_FOLD}/checkpoint_final.pth")" == "${EXPECTED_B_SHA256}" ]] || { echo "Frozen B link SHA256 drifted" >&2; exit 1; }

SUMMARY_SHA="$(s2_uhost_sha256 "${VALIDATION_DIR}/summary.json")"
printf 'completed_at=%s\ncheckpoint_sha256=%s\nsummary_sha256=%s\nnifti_predictions=%s\nnpz_predictions=%s\n' \
    "$(date -u +%FT%TZ)" "${EXPECTED_B_SHA256}" "${SUMMARY_SHA}" "${NIFTI_COUNT}" "${NPZ_COUNT}" > "${COMPLETE_MARKER}"
s2_uhost_require_free_space
echo "S2_FROZEN_B_EVAL_PASS summary_sha256=${SUMMARY_SHA}"
