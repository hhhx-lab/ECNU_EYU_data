import argparse
import numpy as np
from light_training.dataloading.dataset import (
    get_train_val_test_loader_from_train,
    get_train_val_test_loader_seperate,
)
import torch
...
allow_random_split = bool(get_config_value(config, 'allow_random_split', default=False))
...
if __name__ == "__main__":
    ...
    if os.path.isdir(val_data_dir) and os.path.isdir(test_data_dir):
        train_ds, val_ds, test_ds = get_train_val_test_loader_seperate(
            data_dir, val_data_dir, test_data_dir
        )
    elif allow_random_split:
        print("WARNING: using legacy random split because allow_random_split=true")
