...
def ensemble_folders(list_of_input_folders: List[str],
                     ...
                     copy_all_val_files: bool = False,
                     labels_available: bool = False):
    print("#########################")
    print(f"list_of_input_folders: {list_of_input_folders}")
    ...
    if copy_all_val_files:
        print("Copy files active")
        ## Copy all files from all folds for a unique folder (per experiment)
        for folder in list_of_input_folders:
            destination_folder = join(folder, 'all_folds_validation')
            maybe_mkdir_p(destination_folder)
            for fold_n in range(5):
                source_folder = join(folder, f'fold_{fold_n}/validation')
                for file_name in os.listdir(source_folder):
                    source = join(source_folder, file_name)
                    destination = join(destination_folder, file_name)
                    shutil.copy(source, destination)
        shutil.copy(join(list_of_input_folders[0],'dataset.json'), join(list_of_input_folders[0], 'all_folds_validation', 'dataset.json') )
        print("Finished copy files")

    for index, folder in enumerate(list_of_input_folders):
        isdir = os.path.isdir(join(folder, 'all_folds_validation'))
        if isdir:
            print(f"Data from {join(folder, 'all_folds_validation')}")
            list_of_input_folders[index] = join(folder, 'all_folds_validation')

    print(f"len(list_of_input_folders): {len(list_of_input_folders)}")
    ...
    try:
        ...
        print("The file size name is too long")
        print("Create a smaller version of the folder")

        raise Exception("he file size name is too long")

    if len(os.listdir(output_folder)) == 1252:
        raise Exception("All the ensemble done already!")
    ...
    if labels_available:
        ...
        plans_manager = PlansManager(join(list_of_input_folders[0].split('/all_folds_validation')[0], 'plans.json'))
        dataset_json = load_json(join(list_of_input_folders[0].split('/all_folds_validation')[0], 'dataset.json'))
        label_manager = plans_manager.get_label_manager(dataset_json)
        rw = plans_manager.image_reader_writer_class()
        from nnunetv2.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
        from nnunetv2.paths import nnUNet_raw
        print("Computing metrics")
        compute_metrics_on_folder(join(nnUNet_raw, list_of_input_folders[0].split('/')[-3], 'labelsTr'),
                                    output_folder,
                                    join(output_folder, 'summary.json'),
                                    rw,
        ...
        print({
            ...
            'result': load_summary_json(join(output_folder, 'summary.json'))['foreground_mean']['Dice']
            })
...
def entry_point_ensemble_folders():
    ...
    parser.add_argument('-copy_all_val_files',required=False, help='Copy all val files from all folds for a unique folder (per experiment)')
    parser.add_argument('-labels_available',required=False, help='Perform evaluation if labels_available')
    ...
    ensemble_folders(args.i, args.o, args.save_npz, args.np, copy_all_val_files=args.copy_all_val_files, labels_available=args.labels_available)
