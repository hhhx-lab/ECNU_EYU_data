...
def load_generators(args):
    t1c_gen_path = f"{args.t1c_logdir}/weights/generator_{args.g_t1c_n}.pt"
    generator_t1c = get_generator(args=args, MODEL_PATH=t1c_gen_path)
    generator_t1c.eval()
    print(f"{args.t1c_logdir}/weights/generator_{args.g_t1c_n}.pt")

    t1n_gen_path = f"{args.t1n_logdir}/weights/generator_{args.g_t1n_n}.pt"
    generator_t1n = get_generator(args=args, MODEL_PATH=t1n_gen_path)
    generator_t1n.eval()
    print(f"{args.t1n_logdir}weights/generator_{args.g_t1n_n}.pt")

    t2w_gen_path = f"{args.t2w_logdir}/weights/generator_{args.g_t2w_n}.pt"
    generator_t2w = get_generator(args=args, MODEL_PATH=t2w_gen_path)
    generator_t2w.eval()
    print(f"{args.t2w_logdir}/weights/generator_{args.g_t2w_n}.pt")

    t2f_gen_path = f"{args.t2f_logdir}/weights/generator_{args.g_t2f_n}.pt"
    generator_t2f = get_generator(args=args, MODEL_PATH=t2f_gen_path)
    generator_t2f.eval()
    print(f"{args.t2f_logdir}/weights/generator_{args.g_t2f_n}.pt")
    ...
    if torch.cuda.is_available():
        ...
        generator_t2w.cuda()
        generator_t2f.cuda()
    generators_dic = {'scan_t1c':generator_t1c, 'scan_t1n':generator_t1n, 'scan_t2w':generator_t2w, 'scan_t2f':generator_t2f}
...
def generating(args):
    ...
    for ids_scan in label_ids_list:
        ...
        ficheiro_scan_dic={'scan_t1c':registo_lista_scan['scan_t1c'], 'scan_t1n':registo_lista_scan['scan_t1n'],'scan_t2w': registo_lista_scan['scan_t2w'], 'scan_t2f':registo_lista_scan['scan_t2f']}
        healthy_scan_dic={}
...
def __main__():
    ...
    parser.add_argument("--g_t1c_n", type=int, help="Number of the generator t1c")
    parser.add_argument("--g_t1n_n", type=int, help="Number of the generator t1n")
    parser.add_argument("--g_t2w_n", type=int, help="Number of the generator t2w")
    parser.add_argument("--g_t2f_n", type=int, help="Number of the generator t2f")
    parser.add_argument("--t1n_logdir", default="", type=str, help="Directory with the t1n checkpoint")
    parser.add_argument("--t1c_logdir", default="", type=str, help="Directory with the t1c checkpoint")
    parser.add_argument("--t2w_logdir", default="", type=str, help="Directory with the t2w checkpoint")
    parser.add_argument("--t2f_logdir", default="", type=str, help="Directory with the t2f checkpoint")
    ...
    if args.t1n_logdir == "":
        args.t1n_logdir = f"../../Checkpoint/{args.logdir}/t1n"

    if args.t2w_logdir == "":
        args.t2w_logdir = f"../../Checkpoint/{args.logdir}/t2w"

    if args.t1c_logdir == "":
        args.t1c_logdir = f"../../Checkpoint/{args.logdir}/t1c"

    if args.t2f_logdir == "":
        args.t2f_logdir = f"../../Checkpoint/{args.logdir}/t2f"
