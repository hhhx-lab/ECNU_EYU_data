...
def train(args, global_step, train_loader, generator, G_optimizer, recon_loss, w_progression, discriminator, D_optimizer, HOME_DIR, real_label=None, fake_label=None, criterion=None):
    ...
    for step, batch in enumerate(train_loader):
        ...
        for D_n_i in range(args.D_n_update):
            ...
            x_crop_pad = batch["scan_t1c_crop_pad"][0:args.batch_size].cuda() # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch["scan_t1c_noisy"][0:args.batch_size].cuda() # Scan cropped, with padding to 96x96x96 and noise
        ...
        for G_n_i in range(args.G_n_update):
            #batch = next(iter(train_loader))
            # Get data from the batch
            x_crop_pad = batch["scan_t1c_crop_pad"][args.batch_size:].cuda() # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch["scan_t1c_noisy"][args.batch_size:].cuda() # Scan cropped, with padding to 96x96x96 and noise
...
def __main__():
    ...
    while global_step < args.num_steps:
        ...
        save_sample(args=args, image=x_crop_noisy[0], reality="scan_t1c_noisy", iter_num=epoch, path=HOME_DIR)
